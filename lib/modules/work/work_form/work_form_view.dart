...
class WorkFormView extends GetView<WorkFormLogic> with BasePage {
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          controller.isEditing ? s.work_editWork : s.work_createWork,
        ),
      ),
      body: Center(
        child: ElevatedButton(
          onPressed: () async {
            final result = await showDialog<WorkFormResult>(
              context: context,
              builder: (context) => WorkFormDialog(
                existingWork: controller.state.existingWork.value,
              ),
            );
...
class _WorkFormDialogState extends State<WorkFormDialog> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(S.of(context)!.work_pickImageFailed(e.toString()))),
        );
      }
    }
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      Get.back(
        result: WorkFormResult(work: savedWork, isEditing: isEditing),
      );
    } catch (e) {
      if (mounted) {
