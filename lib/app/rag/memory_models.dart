...
/// How the chapter summary was produced.
enum SummarySource { structural, llm }
...
class ChapterSummary {
  const ChapterSummary({
    ...
    this.foreshadowingStatus = '',
    this.emotionalArcs = '',
    this.keyRevelations = '',
    this.summarySource = SummarySource.structural,
  });
  ...
  /// Structured fields populated by LLM summarization (Phase 2).
  final String worldStateChanges;
  final String foreshadowingStatus;
  final String emotionalArcs;
  final String keyRevelations;
  final SummarySource summarySource;

  /// Whether this summary was generated by an LLM.
  bool get isLlmGenerated => summarySource == SummarySource.llm;
  ...
  static ChapterSummary fromJson(Map<String, Object?> json) => ChapterSummary(
    ...
    createdAtMs: _parseInt(json['createdAtMs']),
    worldStateChanges: json['worldStateChanges']?.toString() ?? '',
    foreshadowingStatus: json['foreshadowingStatus']?.toString() ?? '',
    emotionalArcs: json['emotionalArcs']?.toString() ?? '',
    keyRevelations: json['keyRevelations']?.toString() ?? '',
    summarySource: _parseSummarySource(json['summarySource']),
  );
}

SummarySource _parseSummarySource(Object? raw) {
  if (raw == null) return SummarySource.structural;
  return raw.toString() == 'llm' ? SummarySource.llm : SummarySource.structural;
}
