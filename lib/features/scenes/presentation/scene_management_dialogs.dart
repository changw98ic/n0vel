...
Future<void> showChapterDialog(
  ...
  final result = await showDialog<String>(
    ...
    builder: (dialogContext) {
      return DesktopModalDialog(
        title: '编辑章节标签',
        description: '章节标签会影响章节列表分组、工作台路径提示以及阅读模式的章节边界文案。',
        body: SceneDialogField(
          label: '章节标签',
          child: TextField(
            key: fieldKey,
            controller: controller,
            decoration: const InputDecoration(hintText: '例如：第 4 章'),
          ),
        ),
        actions: [
          OutlinedButton(
...
Future<void> showSummaryDialog(
  ...
  final result = await showDialog<String>(
    ...
    builder: (dialogContext) {
      return DesktopModalDialog(
        title: '编辑章节摘要',
        description: '摘要会在章节管理、工作台资源面板和问题跳转提示中复用，应优先概括冲突、线索与当前目标。',
        width: 760,
        body: SceneDialogField(
          label: '章节摘要',
          child: TextField(
            ...
            controller: controller,
            maxLines: 4,
            decoration: const InputDecoration(hintText: '输入章节摘要'),
          ),
        ),
        actions: [
          OutlinedButton(
