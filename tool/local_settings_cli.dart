...
Future<void> main(List<String> args) async {
  ...
  switch (args.first) {
    ...
    case 'decrypt':
      ...
      return;
    case 'export-provider-env':
      await _exportProviderEnv(
        settingsFile: file,
        outputFile: File(
          _optionValue(args, '--output') ??
              '.auto-team/jianlai-refine/providers.env',
        ),
      );
      return;
    default:
...
Future<void> _writeRouted(File file) async {
  ...
  const zhipuBaseUrl = 'https://api.z.ai/api/coding/paas/v4';
...
Future<void> _exportProviderEnv({
  required File settingsFile,
  required File outputFile,
}) async {
  final settings = await loadLocalSettingsObjectFile(file: settingsFile);
  final existing = await _loadExistingEnvFile(outputFile);
  final profiles = _providerProfiles(settings['providerProfiles']);

  final values = <String, String>{};
  void put(String key, List<Object?> candidates) {
  ...
  final ollamaProfile = profiles['ollama-kimi'] ?? profiles['kimi'];
  final mimoProfile = profiles['mimo'];
  put('ZHIPU_API_KEY', [settings['ZHIPU_API_KEY'], settings['apiKey']]);
  put('ZHIPU_BASE_URL', [settings['ZHIPU_BASE_URL']]);
  put('ZHIPU_MODEL', [settings['ZHIPU_MODEL']]);
  put('OLLAMA_API_KEY', [settings['OLLAMA_API_KEY'], ollamaProfile?['apiKey']]);
  put('OLLAMA_BASE_URL', [
    settings['OLLAMA_BASE_URL'],
    ollamaProfile?['baseUrl'],
  ]);
  put('OLLAMA_MODEL', [settings['OLLAMA_MODEL'], ollamaProfile?['model']]);
  put('MIMO_API_KEY', [settings['MIMO_API_KEY'], mimoProfile?['apiKey']]);
  put('MIMO_BASE_URL', [settings['MIMO_BASE_URL'], mimoProfile?['baseUrl']]);
  put('MIMO_MODEL', [settings['MIMO_MODEL'], mimoProfile?['model']]);
  put('DEEPSEEK_API_KEY', [settings['DEEPSEEK_API_KEY']]);
  put('DEEPSEEK_BASE_URL', [settings['DEEPSEEK_BASE_URL']]);
  put('DEEPSEEK_MODEL', [settings['DEEPSEEK_MODEL']]);

  await outputFile.parent.create(recursive: true);
  final keys = values.keys.toList()..sort();
  final buffer = StringBuffer()
    ..writeln(
      '# Generated by tool/local_settings_cli.dart export-provider-env.',
    )
    ..writeln('# This file is local/ignored and may contain API credentials.');
  for (final key in keys) {
    buffer.writeln("$key='${_shellSingleQuote(values[key]!)}'");
  }
  ...
  final summary = keys
      ...
      .map((key) => '$key=set')
      .join(', ');
  stdout.writeln('Provider env written to ${outputFile.path}. $summary');
}

Future<Map<String, String>> _loadExistingEnvFile(File file) async {
...
Map<String, Map<String, Object?>> _providerProfiles(Object? rawProfiles) {
  ...
  for (final rawProfile in rawProfiles) {
    ...
    final normalized = rawProfile.map(
...
Usage:
  ...
  dart run tool/local_settings_cli.dart export-provider-env [--file setting.json] [--output .auto-team/jianlai-refine/providers.env]
...
export-provider-env writes provider variables for batch workers without printing secret values.
''');
}
