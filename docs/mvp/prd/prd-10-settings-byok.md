...
负责配置 `Llm模型服务Config`、管理 自带密钥 连接、执行连接测试，并控制客户端级通用设置。
...
- 保存成功后，新的 `Llm模型服务Config` 从下一次 AI 请求开始生效
...
- 已填写 `接口地址` 与 `模型` 但缺少 `密钥`：进入缺少 密钥 状态，明确提示先补全密钥
- `接口地址` 非法：进入接口地址无效状态，明确提示 URL 格式错误并返回修改
- 模型不在允许列表中：进入模型不受支持状态，禁用测试连接与保存配置，并提示改用受支持模型
- 模型名为空：进入缺少模型状态，禁用测试连接与保存配置，并提示先填写 模型
- 连接超时：进入连接测试失败状态，提供“关闭”与“重新测试”按钮
- `401 / 403`：进入鉴权失败状态，提示检查 密钥、组织权限或账号状态
- `404 / 模型_not_found`：进入模型不存在状态，提示检查模型名拼写或改用可用模型
...
- 保存后的配置在客户端重启后仍可读取
- 密钥 不出现在导出包中
- 未配置连接时，页面应明确提示必须先填写 `密钥 / 接口地址 / 模型`
- 当 `接口地址` 与 `模型` 已填写但 `密钥` 缺失时，页面应进入专门的缺少 密钥 状态
- `接口地址` 非法时，必须阻止保存并展示专门错误状态
- 模型不受支持时，必须阻止测试连接与保存配置，并明确给出建议模型
- 模型名为空时，必须高亮 模型 字段，并同步禁用测试与保存
- 保存成功后，必须出现可感知的成功反馈，不能只做静默保存
- 保存成功后，下一次 AI 请求必须读取新配置，且不得自动重试旧请求
- 测试连接失败不会影响现有项目数据
- 连接测试失败时，必须明确展示失败状态与可重试动作
- `401 / 403`、`404 / 模型_not_found`、`DNS / 网络错误` 必须映射到不同错误状态，不能只用同一张泛化失败页
...
**色彩（§3）**：连接成功使用 `success` 色调；连接失败按错误类型映射到 `accent.danger`、`accent.warning`、`accent.info`；密钥 掩码展示使用 `text.tertiary` 色调。

**图像（§8）**：设置与 自带密钥 页不使用 Candid Lifestyle Photography；全部以结构、排版、层次为主。
...
flowchart TB
    ...
    subgraph Header["顶部轻页眉"]
        Title["设置范围 / 当前 模型服务 摘要"]
    end

    subgraph Body["设置与 自带密钥 页"]
        subgraph Left["左侧 模型服务 配置区"]
            模型服务["provider_name"]
            BaseURL["接口地址"]
            Model["模型"]
            ApiKey["密钥"]
