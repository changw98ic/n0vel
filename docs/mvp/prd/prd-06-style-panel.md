...
用于通过风格问卷或 `风格档案 JSON` 创建 `风格档案`、调整风格强度，并把风格绑定到项目或场景。
...
`风格档案 JSON` 使用固定字段集合，MVP 只识别以下字段：
...
- 问卷缺少必填项：进入缺少必填项状态，禁止生成 `风格档案`，并高亮缺失字段
- 风格校验失败：进入校验失败状态，不生成 `风格档案`，并保留当前风格输入供用户调整后重试
- JSON 配置版本不受支持：进入版本不兼容阻断态，阻止导入，并提示重新导出 1.0 版配置
- JSON 存在未知字段：进入轻提示状态，继续生成 `风格档案`，并明确展示哪些字段已被忽略
...
- JSON 存在未知字段时，必须继续生成 风格档案，但要给出明确的忽略提示
