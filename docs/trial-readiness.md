...
| 步骤 | 字段 | 填写说明 |
| --- | --- | --- |
| 1 | 模型服务 | 给服务起个名字，如 `OpenAI 兼容服务` |
| 2 | 接口地址 | API 接口地址，**注意是否需要以 `/v1` 结尾**（大部分服务需要） |
| 3 | 模型 | 服务商提供的模型 ID，如 `gpt-4o`、`deepseek-chat`，必须和后台完全一致 |
| 4 | 密钥 | 你的密钥 |
| 5 | 连接测试 | 点击测试按钮，通过后再保存 |

设置页还包含“多模型服务配置”和“路由规则”。首次试用只需配置“默认模型”，这些高级选项可以在需要按任务分流时再使用。
...
| --- | --- |
| 连接测试立刻失败 | 密钥是否已填写并保存 |
| 连接超时 | 接口地址是否可达（可在浏览器直接访问试一下）；公司网络是否需要代理 |
| 提示模型不存在 | 模型字段是否和服务商后台的模型 ID 完全一致，注意大小写 |
| 请求成功但返回空 | 接口地址路径是否正确，部分服务不需要 `/v1` 后缀 |
