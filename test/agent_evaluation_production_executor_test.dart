...
void main() {
  ...
  test(
    ...
    () async {
      ...
      await _seedFormalProductionFixture(fixturePath);
  ...
  test(
    ...
    () async {
      ...
      await _seedFormalProductionFixture(path);
...
「沿排水渠走，尽头有第二道门；可他安排的守卫已经在那里，闯进去者从没原路出来。」

「你走前面，我负责让我们有路回来。」调查者拉开铁门，潮湿的黑暗里随即传来枪栓咬合的脆响。

守门人猛地停步：「他们已经来不及退回去——真正的危险就在门后，而我们刚才的每句话，都有人在另一头听着。」''';

Future<void> _seedFormalProductionFixture(String databasePath) async {
  const projectId = _productionFixtureProjectId;
  const sceneId = _productionFixtureSceneId;
  const characterId = 'production-executor-investigator';
    ...
    'scenesByProject': <String, Object?>{
      projectId: <Object?>[
        <String, Object?>{
          ...
          'chapterLabel': '第一章',
          'title': '七号仓门后',
          'summary': '调查者取得账本与备用钥匙线索，并面对仓内伏击。',
        },
      ],
    },
  ...
  await SqliteStoryOutlineStorage(
    dbPath: databasePath,
    ...
    'chapters': <Object?>[
      <String, Object?>{
        'id': 'production-executor-chapter',
        'title': '第一章',
        'summary': '调查者追查七号仓账本。',
        'scenes': <Object?>[
          <String, Object?>{
    ...
    'metadata': <String, Object?>{
      'fixtureRelease': 'production-executor-formal-fixture-v1',
    },
  }, projectId: projectId);
}
