  ...
  List<KnowledgeAtom> knowledge = const [],
    ...
    SceneTaskCard(
      ...
      cast: [
        _castMember(),
        _castMember(
            characterId: 'char-yueren', name: '岳刃', role: '走私联络人'),
      ],
      directorPlan: '目标：逼问\n冲突：顶压',
      beliefs: beliefs,
...
void main() {
  ...
  group('AT-1: Structured roleplay output', () {
    ...
    test('mixed structured and unstructured output extracts only tagged lines',
        () {
      final output = DynamicRoleAgentOutput(
        characterId: 'char-liuxi',
        name: '柳溪',
  ...
  group('AT-3: Capsule-only prompt reinjection', () {
        ...
        capsules: const [
          ContextCapsule(
            intent: RetrievalIntent(
                toolName: 'past_event', query: 'q', purpose: 'p'),
    ...
    test('resolver injects capsule context via dedicated section, not raw data',
        () async {
      ...
      await resolver.resolve(
        ...
        capsules: const [
          ContextCapsule(
            intent: RetrievalIntent(
                toolName: 'relationship', query: 'q', purpose: 'p'),
  ...
  group('AT-4: Belief isolation', () {
    ...
    test(
      ...
      final liuxiTurn =
          RolePlayTurnOutput.fromDynamicAgentOutput(outputs.first);
      final yuerenTurn =
          RolePlayTurnOutput.fromDynamicAgentOutput(outputs.last);
  ...
  group('AT-5: Scene-state resolution', () {
    ...
    test('fallback preserves all role turn actions without silent drops',
        () async {
      final fakeClient = FakeAppLlmClient(
  ...
  group('AT-6: Editor fact discipline', () {
    test(
        'editorial system prompt explicitly prohibits introducing new facts',
        () async {
      ...
      expect(capturedSystemPrompt!, contains('Never contradict'));
  ...
  group('Knowledge visibility filtering', () {
    ...
    test(
        'socialPositionFor returns only the specified character position',
        () {
      final card = _taskCard(
        socialPositions: [
    ...
    test('knowledge atoms are not auto-injected into role agent prompts',
        () async {
      final fakeClient = FakeAppLlmClient(
        responder: (request) {
          final systemPrompt = request.messages.first.content;
  ...
  group('Disclosure policy matching', () {
    ...
    test(
        'deception content alone without target is not deceptive',
        () {
      const state = PresentationState(
        characterId: 'c1',
        surfaceEmotion: '平静',
  ...
  group('Belief update rules', () {
    test('task card beliefs are immutable after construction', () {
      final card = _taskCard(
        beliefs: [
          const CharacterBelief(
              holderId: 'c1', targetId: 'c2', aspect: '信任', value: '高'),
        ],
      );
      expect(
        () => card.beliefs.add(const CharacterBelief(
            holderId: 'c2', targetId: 'c1', aspect: '信任', value: '低')),
      ...
      final card = _taskCard(
        beliefs: [
          const CharacterBelief(
              holderId: 'c1', targetId: 'c2', aspect: '信任', value: '高'),
          const CharacterBelief(
              holderId: 'c1', targetId: 'c3', aspect: '忠诚', value: '中'),
          const CharacterBelief(
              holderId: 'c1', targetId: 'c4', aspect: '意图', value: '低'),
        ],
      );
  ...
  group('Integration: Hidden fact across characters', () {
    test(
      ...
      expect(
          promptsByCharacter['char-yueren'], isNot(contains('仓库铁柜')));
    });
  ...
  // ===========================================================================
  group('Integration: Editor stitches multiple beat types into coherent prose',
      () {
    test(
