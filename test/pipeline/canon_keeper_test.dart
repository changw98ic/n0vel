...
void main() {
  ...
  gate.ProposedWrite proposedWrite(String content) => gate.ProposedWrite(
        tier: MemoryTier.canon,
        content: content,
      );
  ...
  test('consistent write returns empty contradictions', () {
    ...
    final issues = keeper.checkConsistency(proposedWrite('The sky is blue'), facts);
    expect(issues, isEmpty);
  });

  test('exact English negation is detected', () {
    final facts = [canonFact('The door is open')];
    final issues = keeper.checkConsistency(proposedWrite('The door is not open'), facts);
  ...
  test('subject-predicate conflict is detected', () {
    ...
    final issues = keeper.checkConsistency(proposedWrite('Alice is a wizard'), facts);
    expect(issues, hasLength(1));
  ...
  test('numerical conflict is detected', () {
    ...
    final issues = keeper.checkConsistency(proposedWrite('The army has 500 soldiers'), facts);
    expect(issues, hasLength(1));
  ...
  test('non-canon facts are ignored', () {
    ...
    final issues = keeper.checkConsistency(proposedWrite('The door is open'), facts);
    expect(issues, isEmpty);
  });

  test('asWritebackCanonKeeper adapter works with BasicMemoryWritebackGate', () async {
    ...
    final result = await g.validate([
      const gate.ProposedWrite(tier: MemoryTier.canon, content: 'The king is not alive'),
    ]);

    expect(result.accepted, isEmpty);
    expect(result.rejected, hasLength(1));
    expect(result.rejected.first.reasons.first, contains('Canon contradiction'));
  });
}
